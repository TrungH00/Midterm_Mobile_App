...
import com.example.quanlychitieu.adapter.AccountHistoryAdapter;
...
public class SettingAccountsActivity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.account_menuItem_delete) {
            if (uri.getLastPathSegment().equals("1")) {
                AlertDialog.Builder builder = new AlertDialog.Builder(this);
                builder.setMessage("You cannot delete default account")
        ...
        } else if (id == R.id.account_menuItem_edit) {
            if (action.equals(Intent.ACTION_EDIT)) {
                checkUpdateValidity();
            } else {
                insertAccount();
